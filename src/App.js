...
const App = () => {
  ...
  const [insertTodo, { loading: loadingAddTodo }] = useMutation(addTodo, {
    refetchQueries: [getAllTodos, getTodosByUserId],
  });
  const [updateTodoComplete] = useMutation(updateTodo, {
    refetchQueries: [getAllTodos, getTodosByUserId],
  });
  const [deleteTodoById] = useMutation(deleteTodo, {
    refetchQueries: [getAllTodos, getTodosByUserId],
  });
  ...
  const onClickItem = (idx) => {
    const item = dataAllTodo?.todos.find((todo) => todo.id === idx);
    updateTodoComplete({
      variables: {
        id: idx,
  ...
  if (loadingAddTodo || loadingAllTodo || loadingTodoById) {
    return (
    ...
    <>
      ...
      <div className="app">
        <div className="container">
          <h1 className="app-sub">All Todos</h1>

          <div className="form">
            <form className="js-form" onSubmit={onSubmitInput}>
              <input
                autoFocus
                type="text"
        ...
        <div className="container">
          <h1 className="app-sub">Search todo by User ID</h1>
          ...
          <div className="form">
            <form onSubmit={onSubmitSearch}>
              ...
              />
              <input type="submit" value="Search" className="button-form" />
            </form>
          </div>

          <ul className="todo-list js-todo-list">
            {dataTodoById?.todos.map((todo) => (
              <Todo
                key={todo.id}
                id={todo.id}
                title={todo.title}
